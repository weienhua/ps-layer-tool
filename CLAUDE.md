...
│   │   ├── FunctionCanvas.vue  # Canvas 画布（手绘 + 曲线渲染 + 拖拽变形）
...
│   ├── algo/                     # 曲线拟合算法
│   │   ├── curveFit.ts            # 多项式/三角/指数拟合引擎
│   │   └── functionClassify.ts    # 曲线形态分类器
│   ├── types/
...
#### 双模式
- **手绘模式**：Canvas 自由手绘曲线 → 自动拟合解析式
- **表达式模式**：输入表达式 → 生成曲线 → 拖拽变形 → 重拟合

#### 拟合引擎（`src/algo/`）
- `curveFit.ts`：多项式 + 三角函数 + 指数组合拟合，迭代交替逼近
- `functionClassify.ts`：形态分类器，自动推荐基函数组合
